...
\begin{exercise}[6]{$\cO$-Kalkül}
  ...
  \answer{
    ...
    \begin{enumerate}
      ...
      \item[d)] $2^{\sqrt{\log(n)}} = 2^{(\log(n)^{\frac{1}{2}}} = {(2^{log(n)})}^\frac{1}{2} = \sqrt{n} \\
          \lim \limits_{n \to \infty} \frac{n}{\sqrt{n}} = \lim \limits_{n \to \infty} \sqrt{n} = \infty \Rightarrow \sqrt{n} \in \cO(n) \iff n \in \Omega(\sqrt{n}) = \Omega\left(2^{\sqrt{\log(n)}}\right) $
